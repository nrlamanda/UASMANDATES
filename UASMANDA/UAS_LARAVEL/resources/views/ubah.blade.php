...
@section('content')
<div>
    <h1 class="text-center">Ubah Data Pasien Rumah Sakit Laravel</h1>
</div>
<hr>
<hr>
<div class="card text-center">
    <div class="m-5">
        <form action="{{ route('pasien.update', ['pasien' => $pasien->id]) }}" method="post">
            ...
            <div class="form-floating mb-3">
                <input type="string" class="form-control @error('nomor') is-invalid @enderror" id="nomor" name="nomor" value="{{ $pasien->nomor }}">
                @error('nomor')
                    <div class="invalid-feedback">
                        <p>{{ $message }}</p>
                    </div>
